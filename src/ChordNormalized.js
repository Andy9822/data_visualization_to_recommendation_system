...
const PlayerPerDeviceSVG = () => {
  ...
  React.useEffect(async () => {
    ...
    const totals = data.reduce((acc, { source, value }) => {
      if (!acc[source]) acc[source] = 0
      acc[source] += parseInt(value, 10)
      return acc
    }, {})
    ...
    for (const { source, target, value } of data) matrix[index.get(source)][index.get(target)] += (value / totals[source]) * 100; /* eslint-disable-line max-len */
    ...
    svg.append('g')
      ...
      .attr('fill', (d) =>
        ...
        color(names[d.source.index]))
      .attr('d', ribbon)
      .append('title')
      .text((d) => {
        console.log(`totals[${names[d.source.index]}]: ${totals[names[d.source.index]]} => ${names[d.target.index]}: ${d.source.value}`);
        return `${names[d.source.index]} → ${names[d.target.index]} ${d.source.value}`
      });
  }, [])
